...
db.define_table('constancia_modelo',
    ...
    Field('descripcion', type='string', length=200, default="certificado alumno regular", comment=T('tipo de contancia')),
    Field('plantilla', type='text', comment=T('texto base')),
    format= "%(modeloid)s [%(descripcion)s]",
    migrate=migrate)


db.define_table('lalala',
    Field('lalalaid', type='id'),
    ...
    Field('descripcion',type='text',length=200,label=T('certificado de alumno regular')),
    ...
    Field('fechade_entrega', type='date', length=200, label=T('Fecha De Entrega')),   
